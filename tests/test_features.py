...
from corebehrt.classes.features import FeatureCreator  # Replace 'your_module' with the actual module name

class TestFeatureCreator(unittest.TestCase):
    def setUp(self):
        self.concepts = pd.DataFrame({
            'PID': ['1', '1', '2', '2'],
            'TIMESTAMP': [datetime(2020, 1, 1), datetime(2020, 1, 2), datetime(2020, 1, 3), datetime(2020, 1, 4)],
            'CONCEPT': ['A', 'B', 'C', 'D'],
        ...
        self.patients_info = pd.DataFrame({
            'PID': ['1', '2'],
            'BIRTH_DATE': [datetime(1990, 1, 1), datetime(1995, 1, 1)],
            'DEATH_DATE': [datetime(2021, 1, 1), pd.NaT],
            'GENDER': ['M', 'F']
        })
    ...
    def test_create_death(self):
        ...
        self.assertEqual(sum(result['concept'] == 'Death'), 1)  # Only one patient has death info
    ...
    def test_no_ages(self):
        feature_creator = FeatureCreator(ages=False)
        result = feature_creator(self.concepts, self.patients_info)
        self.assertNotIn('age', result.columns)

    def test_no_segments(self):
        feature_creator = FeatureCreator(segment=False)
        result = feature_creator(self.concepts, self.patients_info)
        self.assertNotIn('segment', result.columns)

    def test_no_origin_point(self):
        feature_creator = FeatureCreator(origin_point=None)
        result = feature_creator(self.concepts, self.patients_info)
        self.assertNotIn('abspos', result.columns)
    ...
    def test_origin_point_as_dict(self):
        feature_creator = FeatureCreator(origin_point={'year': 2020, 'month': 1, 'day': 1})
        result = feature_creator(self.concepts, self.patients_info)
        self.assertIn('abspos', result.columns)
