...
def get_bg_for_shard(shard_pids, patients_info, DOB, DOD, BG_cols):
    ...
    # DOD if available
    dod_pids = shard_patients_info[~shard_patients_info[DOD].isna()]['subject_id'].tolist()
    for pid in dod_pids:
...
def get_concepts_for_shard(read_dir, shard_pids, hash_to_integer_map, patients_info):
    ...
    for concept_type in os.listdir(read_dir):
        ...
        numeric_value.extend(concept['RESULT'].round(2) if 'RESULT' in concept.columns else [np.nan] * len(concept))
...
def main_write(
    ...
    finetune_pids = pids[int(n * split["train"]) : int(n * (split["train"] + split["tuning"]))]
    test_pids = pids[int(n * (split["train"] + split["tuning"])) :]

    shard_dict = {}

    for split_name, split_pids in zip(["train", "tuning", "held_out"], [pretrain_pids, finetune_pids, test_pids]):
        shard_dict[split_name] = []
        counter = 0
        for i in range(0, len(split_pids), shard_size):
            shard_pids = split_pids[i : i + shard_size]
            shard_concept = get_concepts_for_shard(read_dir, shard_pids, hash_to_integer_map, patients_info)
