...
class TestTruncationFunctions(unittest.TestCase):
    def setUp(self):
        ...
        self.sample_patient_data_1 = pd.DataFrame(
            {
                "concept": [self.cls_token, self.sep_token]
                + [3] * 10
                + [self.sep_token]
                + [4] * 490
                + [self.sep_token]
    ...
    def test_truncate_patient(self):
        ...
        truncated_patient = truncate_patient(
            self.sample_patient_data_1, self.background_len, max_len, self.vocabulary
        )
    ...
    def test_truncate_patient_keeps_background(self):
        ...
        self.assertTrue(
            (truncated_patient["concept"].iloc[1] == self.sep_token).any(),
    ...
    def test_truncate_patient_does_not_end_with_sep(self):
        ...
        truncated_patient = truncate_patient(
            self.sample_patient_data_3, self.background_len, max_len, self.vocabulary
        )
