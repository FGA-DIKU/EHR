...
class TestSplitFunctions(unittest.TestCase):
    def setUp(self):
        ...
        self.test_data = dd.from_pandas(
            pd.DataFrame(
                {"PID": [1, 1, 1, 2, 2, 2, 3, 4], "Value": [5, 6, 7, 8, 9, 10, 11, 12]}
            ),
            npartitions=1,
        )

        self.large_test_data = dd.from_pandas(
            pd.DataFrame(
                {
    ...
    def test_load_train_val_split(self):
        train_data, val_data = load_train_val_split(
            self.large_test_data, self.split_path
        )
        self.assertEqual(train_data["PID"].nunique().compute(), 3)
