...
class TestCreators(unittest.TestCase):
    ...
    def _create_concepts(self, lab_dict):
        ...
        for concept, values in lab_dict.items():
            for value in values:
                lab_df_list.append({"CONCEPT": concept, "RESULT": value, "TIMESTAMP": pd.Timestamp.now(),
                                    "ADMISSION_ID": next(admission_ids), "PID": next(pids)})
    ...
    def setUp(self):
        ...
        self.patients_info_pd = pd.DataFrame(
            {
                "PID": ["1", "2", "3", "4"],
                "BIRTHDATE": pd.to_datetime(["2000-01-02", "2000-03-20", "2000-05-08", "2000-07-01"]),
        ...
        self.concepts_quantiles = dd.from_pandas(self.concepts_pd_quantiles, npartitions=1)
    ...
    def test_create_binned_value(self):
        ...
        sorted_concepts = list(binned_values.sort_values(by=['index', 'order']).sort_index()["CONCEPT"])
        expected_binned_concepts = [[lab, 'VAL_'+str(int(float(value)*100)) if value != 'Kommentar' else None] for lab, values in self.lab_dict_normed.items() for value in values]        
        expected_flattened_binned_concepts = [item for sublist in expected_binned_concepts for item in sublist if item is not None]
  
        self.assertEqual(sorted_concepts, expected_flattened_binned_concepts)

    def test_normalise_and_create_binned_value(self):
        binned_values = add_binned_values(self.concepts, normalise=ValuesNormalizer.min_max_normalize_results).compute()
        sorted_concepts = list(binned_values.sort_values(by=['index', 'order']).sort_index()["CONCEPT"])
        expected_binned_concepts = [[lab, 'VAL_'+str(int(self._min_max_norm(value, values)*100)) if value != 'Kommentar' else None] for lab, values in self.lab_dict.items() for value in values]        
        expected_flattened_binned_concepts = [item for sublist in expected_binned_concepts for item in sublist if item is not None]
    ...
    def test_create_quantile_value(self):
        ...
        sorted_concepts = list(quantile_values.sort_values(by=['index', 'order']).sort_index()["CONCEPT"])
        expected_quantile_concepts = [[lab, 'VAL_'+value if value.startswith('Q') else None] for lab, values in self.lab_dict_quantiles.items() for value in values]  
        expected_flattened_quantile_concepts = [item for sublist in expected_quantile_concepts for item in sublist if item is not None]
