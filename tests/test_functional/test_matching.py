import unittest
import pandas as pd
from corebehrt.functional.matching import get_col_booleans, startswith_match, contains_match  
...
class TestMatchingFunctions(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.df = pd.DataFrame({
            'CONCEPT': ['COVID', 'COVID_TEST', 'POSITIVE', 'NEGATIVE', 'TEST_COVID', 'OTHER'],
            'VALUE': ['POSITIVE', 'NEGATIVE', 'POSITIVE', 'NEGATIVE', 'POSITIVE', 'NEGATIVE']
        })

    def test_startswith_match_case_sensitive(self):
        # Test startswith_match with case-sensitive matching
        result = startswith_match(self.df, 'CONCEPT', ['COVID', 'TEST'], case_sensitive=True)
    ...
    def test_startswith_match_case_insensitive(self):
        ...
        result = startswith_match(self.df, 'CONCEPT', ['covid', 'test'], case_sensitive=False)
        expected = pd.Series([True, True, False, False, True, False], name='CONCEPT')
        pd.testing.assert_series_equal(result, expected)

    def test_contains_match_case_sensitive(self):
        # Test contains_match with case-sensitive matching
        result = contains_match(self.df, 'CONCEPT', ['COVID', 'TEST'], case_sensitive=True)
        expected = pd.Series([True, True, False, False, True, False], name='CONCEPT')
        print(result.name)
        pd.testing.assert_series_equal(result, expected)
    ...
    def test_contains_match_case_insensitive(self):
        ...
        result = contains_match(self.df, 'CONCEPT', ['covid', 'test'], case_sensitive=False)
        expected = pd.Series([True, True, False, False, True, False], name='CONCEPT')
        pd.testing.assert_series_equal(result, expected)

    def test_get_col_booleans_startswith(self):
        # Test get_col_booleans with startswith matching
        result = get_col_booleans(self.df, ['CONCEPT', 'VALUE'], [['COVID'], ['POSITIVE']], match_how='startswith')
    ...
    def test_get_col_booleans_contains(self):
        ...
        result = get_col_booleans(self.df, ['CONCEPT', 'VALUE'], [['OVID'], ['POSITIVE']], match_how='contains')
        expected = [
