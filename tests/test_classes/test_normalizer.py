...
class TestNormalizer(unittest.TestCase):
    def setUp(self):
        ...
        self.expected_concepts = pd.DataFrame(
            {
                "CONCEPT": ["A", "A", "A", "B", "B", "C", "C"],
                "RESULT": ["VAL_0", "VAL_50", "VAL_100", "VAL_0", "VAL_100", "VAL_UNIQUE", None],
            }
        )
        self.expected_concepts = dd.from_pandas(self.expected_concepts, npartitions=2).compute()

    def test_normalizer(self):
        concepts = ValuesNormalizer.min_max_normalize_results(self.concepts).compute().sort_index()

        # Check if all but the NaN value are as expected
        pd.testing.assert_frame_equal(concepts.iloc[:-1], self.expected_concepts.iloc[:-1], check_dtype=False)

        # Check that NaN value is kept NaN
        self.assertEqual(concepts.iloc[-1]["CONCEPT"], self.expected_concepts.iloc[-1]["CONCEPT"])
