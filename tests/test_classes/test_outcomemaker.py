...
class TestOutcomeMaker(unittest.TestCase):
    def setUp(self):
        ...
        self.concepts_plus = pd.DataFrame({
            ...
            'TIMESTAMP': [pd.Timestamp('2020-01-10'), pd.Timestamp('2020-01-12'), pd.Timestamp('2020-01-12'), pd.Timestamp('2020-01-15')],
        ...
        self.patients_info = pd.DataFrame({
            'PID': ['P1', 'P2', 'P3', 'P4'],
            'info1': [1, 2, 3, 4],
            'TIMESTAMP': [pd.Timestamp('2020-01-10'), pd.Timestamp('2020-01-12'), pd.NaT, pd.Timestamp('2020-01-15')],
        })
    ...
    def test_outcome_maker(self):
        ...
        result = self.outcome_maker(self.concepts_plus, self.patients_info, self.patient_set)
        # Expected outcome for TEST_OUTCOME
        expected_outcome = pd.DataFrame({
            'PID': ['P2'],
            'TIMESTAMP': [pd.Timestamp('2020-01-12'),]
        }, index=[1])
        ...
        pd.testing.assert_frame_equal(result['TEST_OUTCOME'], expected_outcome, check_index_type=False)

        # Expected outcome for TEST_CENSOR
        expected_censor = pd.DataFrame({
            'PID': ['P1'],
            'TIMESTAMP': [pd.Timestamp('2020-01-10')]
        }, index=[0])
        ...
        pd.testing.assert_frame_equal(result['TEST_CENSOR'], expected_censor, check_index_type=False)
