...
class TestExcluder(unittest.TestCase):
    ...
    def test_exclude_short_sequences_list(self):
        ...
        self.assertTrue(all(len(seq) >= self.excluder.min_len + self.excluder.background_length for seq in result))
    ...
    def test_exclude_short_sequences_dict(self):
        ...
        self.assertTrue(all(len(seq) >= self.excluder.min_len + self.excluder.background_length for seq in result['concept']))
        self.assertEqual(len(kept_indices), 2)
