...
def get_codes_to_prune(code_counts, min_counts, strategy="individual"):
    if strategy == "individual":
        ...
        codes_to_prune = [
            ...
            if count < min_counts and len(code) > 2
        ]
    elif strategy == "group":
...
def prune_code_mappings(
    code_counts, code_mappings, min_counts, strategy="individual", iterations=0
):
    ...
    for code in codes_to_prune:
        ...
        if code.startswith("DC50"):
            ...
            print(
                f"Pruning {code} (count: {code_counts[code]}) to {prefix} (count: {prefix_count})"
            )
    ...
    return prune_code_mappings(
        code_counts, code_mappings, min_counts, strategy, iterations + 1
    )
...
def get_code_mapping(code_counts, min_count, strategy="individual"):
    ...
    code_counts, code_mappings = prune_code_mappings(
        code_counts, code_mappings, min_count, strategy
    )
...
if __name__ == "__main__":
    mount_context = setup_azure(
        workspacename="researcher_data", folder_path=FOLDER_PATH
    )
    ...
    for i, (code, count) in enumerate(
        sorted(code_counts.items(), key=lambda x: x[1], reverse=True)[:10]
    ):
        print(f"  {code}: {count}")
    ...
    dc50_mappings = {
        code: mapped
        for code, mapped in group_mapping.items()
        if code.startswith("DC50")
    }
