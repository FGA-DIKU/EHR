...
def add_label_noise(data: pd.DataFrame, switch_percentage: float = 0.05) -> pd.DataFrame:
    ...
    positive_patients = set(noisy_data[noisy_data['is_positive']]['subject_id'].unique())
    negative_patients = set(noisy_data[~noisy_data['is_positive']]['subject_id'].unique())
...
def remove_random_data(data: pd.DataFrame, remove_percentage: float = 0.10) -> pd.DataFrame:
    """
    ...
    lab_indices_to_remove = np.random.choice(lab_data.index, n_lab_records_to_remove, replace=False)
...
def create_distribution_plot(data: pd.DataFrame, save_path: Path, perfect_roc: float, title_suffix: str = "") -> None:
    """
...
def create_comparison_plot(original_data: pd.DataFrame, noisy_data: pd.DataFrame, save_path: Path) -> None:
    """
...
def calculate_theoretical_performance(data: pd.DataFrame) -> dict:
    ...
    if n_total_patients > 0:
        ...
        overall_sweep_auc = (n_patients_with_labs * sweep_auc + n_patients_without_labs * 0.5) / n_total_patients
        overall_scipy_auc = (n_patients_with_labs * scipy_mann_whitney_u_auc + n_patients_without_labs * 0.5) / n_total_patients
    else:
    ...
    print(f"Patients with lab data: {n_patients_with_labs}/{n_total_patients} ({n_patients_with_labs/n_total_patients*100:.1f}%)")
    print(f"Patients without lab data: {n_patients_without_labs}/{n_total_patients} ({n_patients_without_labs/n_total_patients*100:.1f}%)")
...
def main():
    ...
    create_distribution_plot(noisy_data, save_path, noisy_performance["sweep_auc"], f" (Noise: {noise_suffix})")
    
    # Create comparison plot
    comparison_save_path = Path(DEFAULT_PLOT_DIR) / f"{output_filename}_comparison_plot.png"
