...
def generate_timestamps(pids_list: List[str], patient_df: pd.DataFrame, concepts: List[str]) -> List[pd.Timestamp]:
    """
    ...
    for i, (pid, concept) in enumerate(zip(pids_list, concepts)):
        ...
        birthdate = pd.Timestamp(year=2016, month=1, day=1) # pd.to_datetime(patient_info["birthdate"])
...
def generate_data(patient_df: pd.DataFrame, write_dir: str) -> None:
    ...
    print(f"Percentage with diagnosis: {patients_with_diagnosis/total_patients*100:.2f}%")
    ...
    data["time"] = generate_timestamps(data["subject_id"].tolist(), patient_df, data["code"].tolist())
