...
SAVE_NAME = f'gaus_mean_{LOW_MEAN}_{HIGH_MEAN}_std_{STD}_n{N}_50p_1unq'
...
def create_distribution_plot(data: pd.DataFrame, save_path: Path, perfect_roc: float) -> None:
    """
...
def main():
    ...
    create_distribution_plot(modified_data, save_path, perfect_roc = theoretical_performance["sweep_auc"])
    ...
    if lab_mask.any():
        ...
        if max_val > min_val:
            normalized_data.loc[lab_mask, 'numeric_value'] = (
                (normalized_data.loc[lab_mask, 'numeric_value'] - min_val) / (max_val - min_val)
            )
        else:
    ...
    create_distribution_plot(normalized_data, save_path, perfect_roc = normalized_theoretical_performance["sweep_auc"])
