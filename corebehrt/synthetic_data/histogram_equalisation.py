...
def apply_histogram_equalisation(data: pd.DataFrame, lab_code: str = 'S/LAB1', n_bins: int = 256) -> pd.DataFrame:
    """
...
def create_comparison_plot(original_data: pd.DataFrame, equalised_data: pd.DataFrame, save_path: Path) -> None:
    """
    ...
    equalised_positive = equalised_data[lab_mask & equalised_data['is_positive']]['numeric_value']
    equalised_negative = equalised_data[lab_mask & ~equalised_data['is_positive']]['numeric_value']
    ...
    ax2.hist(equalised_positive, bins=30, alpha=0.7, label='With Diagnosis', color='red')
...
def main():
    ...
    create_distribution_plot(equalised_data, save_path, equalised_performance["sweep_auc"], " (Histogram Equalised)")
