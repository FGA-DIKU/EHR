...
class Utilities:
    ...
    @staticmethod
    def select_and_order_outcomes_for_patients(
        all_outcomes: Dict, pids: List, outcome: Union[str, dict]
    ) -> List:
        ...
        if isinstance(outcome, str):
            ...
            outcome, dict
        ):  # For temporal censoring (censoring equally at a date)
            outcome_datetime = datetime(**outcome)
            logger.warning(
                f"Using {ORIGIN_POINT} as origin point. Check whether it is the same as used for feature creation."
            )
            outcome_abspos = Utilities.get_abspos_from_origin_point(
                [outcome_datetime], ORIGIN_POINT
            )
            outcome_group = outcome_abspos * len(outcome_pids)
        else:
            raise ValueError(f"Unknown outcome type {type(outcome)}")
        assert len(outcome_pids) == len(
            outcome_group
        ), "Mismatch between PID_KEY length and outcome_group length"
        ...
        if not set(pids).issubset(outcome_pids):
            logger.warn(
                f"PIDs is not a subset of outcome PIDs, there is a mismatch of {len(set(pids).difference(outcome_pids))} patients"
            )

        outcomes = [
            outcome_group[pid_to_index[pid]] if pid in outcome_pids else None
            for pid in pids
        ]
        return outcomes
    ...
    def get_abspos_from_origin_point(
        timestamps: Union[pd.Series, List[datetime]], origin_point: Dict[str, int]
    ...
    def check_and_adjust_max_segment(data: Data, model_cfg: Config) -> None:
        ...
        if max_segment >= type_vocab_size:
            logger.warning(
                f"You've set type_vocab_size too low. Max segment {max_segment} >= type_vocab_size {type_vocab_size}\
    ...
    def get_gender_token(vocabulary: dict, gender_key: str) -> int:
        ...
        for category, keys in BG_GENDER_KEYS.items():
            ...
            raise ValueError(
                f"Unknown gender {gender_key}, please select one of {BG_GENDER_KEYS}"
            )
        ...
        raise ValueError(
            f"None of BG_GENDER_+{BG_GENDER_KEYS[gender_category]} found in vocabulary."
        )
    ...
    @staticmethod
    def select_and_reorder_feats_and_pids(
        feats: Dict[str, List], pids: List[str], select_pids: List[str]
    ) -> Tuple[Dict[str, List], List[str]]:
        """Reorders pids and feats to match the order of select_pids"""
        if not set(select_pids).issubset(set(pids)):
            raise ValueError(
                f"There are {len(set(select_pids).difference(set(pids)))} select pids not present in pids"
            )
        pid2idx = {pid: index for index, pid in enumerate(pids)}
        indices_to_keep = [
            pid2idx[pid] for pid in select_pids
        ]  # order is important, so keep select_pids as list
        selected_feats = {}
    ...
    def calculate_ages_at_censor_date(data: Data) -> List[int]:
        ...
        for abspos, age, censor_date in zip(
            data.features["abspos"], data.features["age"], data.censor_outcomes
        ):
            if censor_date is None:
                ages_at_censor_date.append(
                    age[-1]
                )  # if no censoring, we take the last age
                continue
            ...
            age_at_censor = (
                age[closest_abspos_index]
                + time_differences_h[closest_abspos_index] / 24 / 365.25
            )
