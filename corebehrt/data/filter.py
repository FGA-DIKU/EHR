...
class PatientFilter:
    ...
    def select_censored(self, data: Data) -> Data:
        ...
        kept_indices = [
            i for i, censor in enumerate(data.censor_outcomes) if pd.notna(censor)
        ]
        return self.select_entries(data, kept_indices)
    ...
    def filter_by_min_sequence_length(self, data: Data) -> Data:
        ...
        _, kept_indices = exclude_short_sequences(
            data.features,
            min_len=self.cfg.data.get("min_len", 3),
            background_length=background_length,
        )  # ! this needs to be improved, we are filtering features twice
        return self.select_entries(data, kept_indices)
