...
class InvalidTypeError(Exception):
    def __init__(self, tp, valid_tps, name=None):
        super().__init__(
            f"Invalid parameter type: expected on of {valid_tps}, but got type {tp}"
            + ("" if name is None else f" for parameter '{name}'")
        )
        ...
        if tp in (list, set):
            ...
            if index_name is not None and inp.index.name != index_name:
                raise InvalidDataFrameError(
                    f"Invalid index name: expected {index_name}, but got {inp.index.name}"
                    + ("" if name is None else f" for parameter {name}")
                )
