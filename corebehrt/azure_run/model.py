...
class Model:
    ...
    def register(self, name=None, exist_ok=False, path=None, datasets=[]):
        ...
        if name is None and self.name() is None:
            raise Exception(
                "Cannot register model - no name provided and model is not previously named!"
            )
        ...
        if not exist_ok:
            try:
                ...
                raise Exception(
                    f"Model {self.name()} already registered, version = {meta.version()}!"
                )
            except:
        ...
        if self.path() is None:
            raise Exception(
                f"Cannot register an unsaved model! Please call save beforehand or provide a path..."
            )
