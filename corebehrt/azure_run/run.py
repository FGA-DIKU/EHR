...
class Run:
    ...
    def log_evaluation(Y, Pr, name="Evaluation", num_thresholds=40):
        ...
        if R.remote is not None:
            ...
            value = {
                ...
                "data": {
                    "probability_tables": [[_cm(t, False) for t in pr_ts]],
                    "percentile_tables": [[_cm(t, False) for t in pe_ts]],
    ...
    def log_plot(plt, name, filename=None):
        ...
        if R.remote is not None:
            ...
            log().warning(
                f"Attempted to log plot to non-run - skipping. Provide 'filename' to save as file instead."
            )
