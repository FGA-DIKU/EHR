...
def get_max_retry_folder(folders):
    ...
    retry_numbers = [
        int(re.search(pattern, folder).group(1))
...
def save_to_blobstore(local_path: str, remote_path: str, overwrite: bool = False):
    ...
    try:
        ...
        output_path = (
            "outputs" if retry_folder is None else join("outputs", retry_folder)
        )
...
class AzurePathContext:
    ...
    def setup_run_and_mount_context(self) -> Tuple:
        ...
        if self.azure_env:
            run, mount_context = setup_azure(
                self.cfg.paths.run_name, dataset_name=self.dataset_name
            )
        return run, mount_context
    ...
    def adjust_paths_for_azure_pretrain(self) -> Tuple:
        ...
        """
        if self.azure_env:
            self.cfg.paths.data_path = self._prepend_mount_point(
                self.cfg.paths.data_path
            )
            if "predefined_splits" in self.cfg.paths:
                self.cfg.paths.predefined_splits = self._prepend_mount_point(
                    self.cfg.paths.predefined_splits
                )
            if self.cfg.paths.get("model_path", None) is not None:
                self.cfg.paths.model_path = self._prepend_mount_point(
                    self.cfg.paths.model_path
                )
    ...
    def azure_onehot_setup(self) -> Tuple:
        ...
        if self.azure_env:
            self.cfg.paths.finetune_features_path = self._prepend_mount_point(
                self.cfg.paths.finetune_features_path
            )
    ...
    def azure_data_pretrain_setup(self) -> Tuple:
        ...
        if self.azure_env:
            self.cfg.loader.data_dir = self._prepend_mount_point(
                self.cfg.loader.data_dir
            )
            if "vocabulary" in self.cfg.paths:
                self.cfg.paths.vocabulary = self._prepend_mount_point(
                    self.cfg.paths.vocabulary
                )
            if "predefined_splits_dir" in self.cfg:
                self.cfg.predefined_splits_dir = self._prepend_mount_point(
                    self.cfg.predefined_splits_dir
                )
            if "exclude_pids" in self.cfg and self.cfg["exclude_pids"]:
                self.cfg.exclude_pids = self.prepend_mount_point_to_paths(
                    self.cfg["exclude_pids"]
                )
            if "assigned_pids" in self.cfg and self.cfg["assigned_pids"]:
                self.cfg.assigned_pids = self.prepend_mount_point_to_assigned_pids(
                    self.cfg["assigned_pids"]
                )
            self._handle_outputs_path()
        return self.cfg, self.run, self.mount_context
    ...
    def azure_encode_setup(self) -> Tuple:
        ...
        if self.azure_env:
            ...
            self.cfg.paths.outcomes_path = self._prepend_mount_point(
                self.cfg.paths.outcomes_path
            )
            self._handle_outputs_path()
        return self.cfg, self.run, self.mount_context
    ...
    def _handle_outputs_path(self) -> None:
        ...
        if self.cfg.paths.get("output_path", None) is None:
            ...
            if not self.cfg.paths.output_path.startswith(OUTPUTS_DIR):
                self.cfg.paths.output_path = join(
                    OUTPUTS_DIR, self.cfg.paths.output_path
                )
    ...
    def prepend_mount_point_to_assigned_pids(
        self, assigned_pids: Dict[str, Union[str, List[str]]]
    ...
    def prepend_mount_point_to_paths(
