...
class Saver:
    ...
    def save_sequence_lengths(self, data: Data) -> Data:
        ...
        if not data.outcomes:
            sequence_lens = torch.tensor(
                [len(concepts) for concepts in data.features["concept"]]
            )
            torch.save(
                sequence_lens,
                join(self.run_folder, f"sequences_lengths_{data.mode}.pt"),
            )
            return data
        else:
            pos_indices = set(
                [i for i, outcome in enumerate(data.outcomes) if not pd.isna(outcome)]
            )
            sequence_lens_neg = torch.tensor(
                [
                    len(concepts)
            ...
            sequence_lens_pos = torch.tensor(
                [
                    len(concepts)
                    for i, concepts in enumerate(data.features["concept"])
            ...
            torch.save(
                sequence_lens_neg,
                join(self.run_folder, f"sequences_lengths_{data.mode}_neg.pt"),
            )
            torch.save(
                sequence_lens_pos,
                join(self.run_folder, f"sequences_lengths_{data.mode}_pos.pt"),
            )
            return data
    ...
    def save_patient_nums(
        self, train_data: Data = None, val_data: Data = None, folder: str = None
    ) -> None:
        ...
        train_df = pd.DataFrame(
            {
                "train": [
                    len(train_data),
                    len([t for t in train_data.outcomes if not pd.isna(t)]),
                ]
            },
        ...
        val_df = pd.DataFrame(
            {
                "val": [
                    len(val_data),
                    len([t for t in val_data.outcomes if not pd.isna(t)]),
                ]
            },
            index=["total", "positive"],
        )
        ...
        patient_nums.to_csv(
            join(self.run_folder if folder is None else folder, "patient_nums.csv"),
            index_label="Patient Group",
        )

    def save_patient_nums_general(
        self, data_dic: Dict[str, Data], folder: str = None
    ) -> None:
    ...
    def save_data(self, data: Data) -> None:
        ...
        if data.censor_outcomes is not None:
            torch.save(
                data.censor_outcomes, join(self.run_folder, "censor_outcomes.pt")
            )
