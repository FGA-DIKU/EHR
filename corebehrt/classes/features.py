...
config.set({"dataframe.convert-string": False}) # Avoids dask greedy string conversion of objects
...
class FeatureCreator:
    ...
    def __call__(
        ...
        if "RESULT" in concepts.columns:
            concepts = ValuesNormalizer.min_max_normalize_results(concepts, num_bins=self.value_creator_kwargs.get("num_bins", 100))

        # Prepare dataframes
        concepts = self.prepare_concepts(concepts, patients_info)
        background = create_background(patients_info, self.background_vars, cls_token=self.cls_token)
    ...
    def prepare_concepts(
        ...
        def join_with_patients_info(concepts_partition: dd.DataFrame) -> dd.DataFrame:
            ...
            return concepts_partition.merge(patients_info, how="left", left_index=True, right_index=True)
    ...
    def combine_to_event(dataframe: dd.DataFrame) -> dd.DataFrame:
        ...
        dataframe["concept"] = dataframe[event_columns].map_partitions(lambda x: x.values.tolist())
