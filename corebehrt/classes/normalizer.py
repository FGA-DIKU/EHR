...
class ValuesNormalizer:
    ...
    def min_max_normalize_results(values: dd.DataFrame) -> dd.DataFrame:
        ...
        min_values = grouped.min().compute().drop(columns=["TIMESTAMP"]).rename(columns={"RESULT": "min"})        
        max_values = grouped.max().compute().drop(columns=["TIMESTAMP"]).rename(columns={"RESULT": "max"})        
        count_values = grouped.count().compute().drop(columns=["TIMESTAMP"]).rename(columns={"RESULT": "count"})   
        min_max_count = dd.merge(min_values, max_values, left_index=True, right_index=True).merge(count_values, left_index=True, right_index=True)
        values = dd.merge(values, min_max_count, left_on="CONCEPT", right_index=True, how="left")

        # Apply normalisation row-wise
        values["RESULT"] = values.apply(lambda row: ValuesNormalizer.apply_normalization(row, min_max_normalize, min_value=row["min"], max_value=row["max"], min_count=3), axis=1, meta=('RESULT', 'f8'))
