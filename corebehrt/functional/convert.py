...
def convert_to_sequences(features: dd.DataFrame) -> Tuple[Dict[str, List], List[str]]:
    ...
    features_dict = {col: grouped[col].apply(list).to_dict() for col in features.columns if col != 'PID'}
    
    for col in features_dict: # Convert dictionary of lists from grouped structure to normal lists
